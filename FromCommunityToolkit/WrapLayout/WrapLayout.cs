...
/* https://github.com/CommunityToolkit/WindowsCommunityToolkit/tree/rel/7.1.2/Microsoft.Toolkit.Uwp.UI.Controls.Primitives/WrapLayout */

namespace ItemsRepeaterVirtualizationBug.FromCommunityToolkit
{
    /// <summary>
    ...
    public class WrapLayout : VirtualizingLayout
    {
        ...
        private void StartCollectingMessages(string info)
        {
            messages = new List<string> { info };
        }

        private void AddElementMessage(object element, string info)
        {
            if (messages == null) throw new InvalidOperationException();

            var itemControl = element as ItemControl;

            if (itemControl != null) messages.Add(info + itemControl.HorizontalLinesText);
            else messages.Add(info + "??");
        }

        private void FinishCollectingMessages()
        {
            if (messages == null) throw new InvalidOperationException();

            Logger.WriteLine(string.Join(", ", messages));
            messages = null;
        }
        #endregion
        ...
        private static void LayoutPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                Logger.WriteLine("LayoutPropertyChanged");
        ...
        {
            Logger.WriteLine("InitializeForContextCore");
        ...
        {
            Logger.WriteLine("UninitializeForContextCore");
        ...
        {
            Logger.WriteLine(String.Format("OnItemsChangedCore source:{0} args.Action:{1}", source, args.Action));
            ...
            switch (args.Action)
            {
                ...
                case NotifyCollectionChangedAction.Reset:
                    ...
                    if (RecycleOnReset)
                    {
                        Logger.WriteLine("RecycleAllElements");
                        state.RecycleAllElements();
                    }
        ...
        {
            StartCollectingMessages(string.Format("MeasureOverride context.ItemCount:{0}", context.ItemCount));
            ...
            for (int i = 0; i < context.ItemCount; i++)
            {
                ...
                if (item.Measure == null)
                {
                    ...
                    AddElementMessage(item.Element, string.Format("@{0}>", i)); /* minesworld */
                }
                ...
                if (vEnd < realizationBounds.VMin)
                {
                    ...
                    {
                        AddElementMessage(item.Element, "R>"); /* minesworld */
            ...
            FinishCollectingMessages(); /* minesworld */
        ...
        {
            /* Logger.WriteLine(String.Format("ArrangeOverride finalSize:{0}", finalSize)); */
